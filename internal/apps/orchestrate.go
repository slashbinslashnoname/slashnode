...
func installOne(dir, appID string, provided, imageTagOverride map[string]string, isTarget bool, out io.Writer) error {
	...
	for _, in := range man.Inputs {
		...
		if !ok || val == "" {
			switch {
			case in.GenerateJWT != "":
				// Derived key (e.g. Supabase anon/service): sign an HS256 JWT with a
				// secret generated by an earlier input. That input must be listed
				// before this one so it's already in the secret map.
				signing := secret[in.SignWith]
				if signing == "" {
					return fmt.Errorf("%s: signing secret %q not available (declare %s with generate before %s)", in.Key, in.SignWith, in.SignWith, in.Key)
				}
				if val, err = signSupabaseJWT(signing, in.GenerateJWT); err != nil {
					return err
				}
			case in.Default != nil:
